#!/usr/bin/env tsx
/**
 * Production Blog Setup Script
 * Sets up blog schema and imports posts for production environment
 */
...
async function setupProductionBlog() {
  console.log("🚀 Setting up blog for production environment...");
  
  const supabase = createProdAdminClient();
  
  try {
    console.log("📋 Step 1: Checking if blog tables exist...");
    ...
    if (catError) {
      console.error("❌ Blog tables don't exist in production database");
      console.log("📝 You need to apply the blog schema manually:");
      console.log("1. Go to https://supabase.com/dashboard");
      console.log("2. Navigate to SQL Editor");
      console.log("3. Copy and paste the content of blog-schema-setup.sql");
      console.log("4. Run the migration");
      console.log("5. Then run this script again");
      return;
    }
    
    console.log("✅ Blog tables exist in production");
    ...
    const { data: allCategories, error: getAllCatError } = await supabase
      .from("blog_categories")
      .select("id, slug");
    
    if (getAllCatError) {
      throw new Error(`Failed to fetch categories: ${getAllCatError.message}`);
    }
    
    const categoryMap = new Map(allCategories.map(c => [c.slug, c.id]));
    
    console.log("📋 Step 3: Importing blog posts...");
    ...
    for (const post of blogPosts) {
      ...
      if (insertError) {
        console.error(`❌ Failed to import "${post.title}":`, insertError.message);
      } else {
    ...
    console.log(`📊 Results:`);
    console.log(`   • ${importedCount} posts imported`);
    console.log(`   • ${skippedCount} posts already existed`);
    ...
    console.log(`📈 Total published posts in production: ${count}`);
    console.log(`🌐 Blog should now be live at: https://wildwestslc.com/blog`);
...
// Run the setup
setupProductionBlog().catch(console.error);